...
function Notifications() {
	...
	if (notifs && notifs.items.length > 0) {
			...
			if (object.type === followType) {
				...
				allNotifs.push(
					...
					<div key={keyIndex} className="flex flex-initial rounded-md shadow-md bg-orange-300/75 p-3 m-2">
						<p className="text-sm">
							<Link href={author.url} className="font-semibold hover:underline hover:text-orange-700">
							{author.displayName}
							</Link>
							{" "} wants to follow you.
						</p>
				...
				allNotifs.push(	
					<div key={keyIndex} className="flex flex-initial rounded-md shadow-md bg-orange-300/75 p-3 m-2">
						<p className="text-sm">
							...
							{" "}left a like on your {" "}
							<Link href={object.object} className="font-semibold hover:underline hover:text-orange-700">
								post
							</Link>
							.
						</p>
				...
				allNotifs.push(	
					<div key={keyIndex} className="flex flex-initial rounded-md shadow-md bg-orange-300/75 p-3 m-2">
						<p className="text-sm">
							...
							<Link href={object.id} className="font-semibold hover:underline hover:text-orange-700">
								post
							</Link>
							.
						</p>
		...
		allNotifs.push(
			<div key="1" className="flex flex-initial rounded-md shadow-md bg-gray-300/75 p-3 m-2">
				<p className="text-sm">
					No Notifications.
				</p>
			</div>
